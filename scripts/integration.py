...
def save_to_file(position_x, age, v_value, speed_kmh):
    try:
        path = os.path.expanduser("~/jinwoo_ws/marker_data2.txt")
        with open(path, 'a') as f:
            f.write(f"{position_x}, {age}, {v_value}, {speed_kmh}\n")
        rospy.loginfo(f"✅ 저장 완료: {path}")
    except Exception as e:
        rospy.logerr(f"❌ 파일 저장 실패: {e}")
...
class Integration:
    def __init__(self):
        ...
        rospy.Subscriber('/novatel/oem7/inspva', INSPVA, self.novatel_cb)
        rospy.Subscriber('/novatel/oem7/inspva2', INSPVA, self.novatel_cb2)
        rospy.Subscriber('/mobinha/visualize/visualize/track_text', MarkerArray, self.callback)
    ...
    def novatel_cb(self, msg):
        ...
        # 속도 (m/s) -> (km/h)로 변환하여 파일에 저장
        speed_kmh = v * 3.6
        self.save_speed = speed_kmh  # 속도를 저장

        speed_marker = create_text_marker(target_frame, "Speed: {:.2f} km/h".format(speed_kmh), self.timestamp, Point(0, 0, 3.0), EGO_CAR_COLOR)
        self.pub_ego_speed_marker.publish(speed_marker)
    ...
    def callback(self, msg):
        rospy.loginfo("📥 callback triggered!")  # 이 로그가 나오면 콜백이 호출된 것
        ...
        try:
            for marker in msg.markers:
                ...
                if v_match:
                    ...
                    # novatel_cb에서 계산된 x 좌표를 사용하여 'Position' 값 저장
                    position_x =marker.pose.position.x # novatel_cb에서 계산된 x 좌표 사용
                    # save_to_file(position_x, age, v_value, self.save_speed)  # Position, Age, V, Speed 값을 파일에 저장
